...
export default function About() {
  ...
  return (
    <section id="sobre-nosotros" ref={ref} style={{ padding: '6rem 2rem', background: 'var(--dark-2)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(460px, 1fr))', gap: '4rem', alignItems: 'center' }}>
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7 }}>
            ...
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 2.8rem)', fontWeight: 800, lineHeight: 1.2, marginBottom: '1.25rem', letterSpacing: '-0.02em' }}>
              {t('about.headline1')}{' '}
          ...
          >
            {values.map((v, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 + i * 0.1 }}
                style={{ padding: '1.5rem', borderRadius: 16, background: 'var(--glass)', border: '1px solid var(--glass-border)', transition: 'border-color 0.2s, transform 0.2s', cursor: 'default' }}
              ...
              >
                <div style={{ width: 42, height: 42, borderRadius: 10, background: 'linear-gradient(135deg, rgba(108,99,255,0.2), rgba(0,212,170,0.15))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary-light)', marginBottom: '0.75rem' }}>
                  {ICONS[i]}
                </div>
