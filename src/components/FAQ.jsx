...
export default function FAQ() {
  ...
  return (
    <section id="faq" ref={ref} style={{ padding: '6rem 1.5rem', background: 'var(--dark-2)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'clamp(280px, 50vw, 600px)', height: 'clamp(280px, 50vw, 600px)', background: 'radial-gradient(circle, rgba(0,212,170,0.06) 0%, transparent 65%)', pointerEvents: 'none' }} />

      <div style={{ maxWidth: 760, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          {items.map((item, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: i * 0.06 }}
                style={{ borderRadius: 14, background: 'var(--dark-3)', border: `1px solid ${isOpen ? 'rgba(0,212,170,0.3)' : 'var(--glass-border)'}`, overflow: 'hidden', transition: 'border-color 0.25s' }}
              >
                <button
                  onClick={() => setOpen(isOpen ? null : i)}
                  style={{ width: '100%', padding: 'clamp(0.9rem, 2.5vw, 1.25rem) clamp(1rem, 3vw, 1.5rem)', background: 'none', border: 'none', color: 'var(--text)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', textAlign: 'left', fontFamily: 'Inter, sans-serif' }}
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.65rem', flex: 1, minWidth: 0 }}>
                    <HelpCircle size={15} color={isOpen ? 'var(--accent)' : 'var(--text-muted)'} style={{ marginTop: 2, flexShrink: 0, transition: 'color 0.2s' }} />
                    <span style={{ fontWeight: 600, fontSize: 'clamp(0.85rem, 2vw, 0.95rem)', lineHeight: 1.4 }}>{item.q}</span>
                  </div>
                  <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.25 }} style={{ flexShrink: 0 }}>
                    <ChevronDown size={17} color={isOpen ? 'var(--accent)' : 'var(--text-muted)'} />
                  </motion.div>
                </button>
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div style={{ padding: '0 clamp(1rem, 3vw, 1.25rem) clamp(0.9rem, 2.5vw, 1.25rem) clamp(1rem, 3vw, 1.5rem)', color: 'var(--text-muted)', fontSize: '0.88rem', lineHeight: 1.8 }}>
                        {item.a}
                      </div>
                    </motion.div>
                  )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.5 }}
          style={{ marginTop: '2rem', padding: 'clamp(1.25rem, 4vw, 1.75rem) clamp(1.25rem, 4vw, 2rem)', borderRadius: 18, background: 'linear-gradient(135deg, rgba(108,99,255,0.1), rgba(0,212,170,0.08))', border: '1px solid rgba(108,99,255,0.2)', textAlign: 'center' }}
        >
