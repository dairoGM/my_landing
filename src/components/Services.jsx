...
import { Globe, Smartphone, Database, Settings, BarChart3, ShieldCheck, CheckCircle2, ArrowRight } from 'lucide-react'
import { useTranslation } from 'react-i18next'

const ICONS  = [<Globe size={26} />, <Smartphone size={26} />, <Database size={26} />, <Settings size={26} />, <BarChart3 size={26} />, <ShieldCheck size={26} />]
...
export default function Services() {
  ...
  return (
    <section id="servicios" ref={ref} style={{ padding: '6rem 1.5rem', background: 'var(--dark-2)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="grid-auto">
          {items.map((s, i) => (
            ...
            >
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', lineHeight: 1.7, marginBottom: '1.25rem', flex: 1 }}>{s.desc}</p>

              {/* Deliverables */}
              {s.deliverables && (
                <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                  {s.deliverables.map((d, j) => (
                    <li key={j} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.82rem', color: 'var(--text-muted)' }}>
                      <CheckCircle2 size={14} color={COLORS[i]} style={{ marginTop: 2, flexShrink: 0 }} />
                      {d}
                    </li>
                  ))}
                </ul>
              ...
              <a href="#contacto"
                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.82rem', fontWeight: 600, color: COLORS[i], textDecoration: 'none', marginTop: 'auto' }}
              >
